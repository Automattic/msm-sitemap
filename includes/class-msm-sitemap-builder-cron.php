<?php
/**
 * MSM_Sitemap_Builder_Cron class
 *
...
class MSM_Sitemap_Builder_Cron {
	...
	public static function action_reset_data() {
		// Do the same as when we finish then tell use to delete manually rather than remove all data.
		self::reset_sitemap_data();
		Metro_Sitemap::show_action_message(
			sprintf(
				/* translators: 1: Post type slug, 2: WP-CLI command */
	 ...
	 *
	 * @param array $date
	 * @param int   $time Unix timestamp (UTC) for when to next run the event.
	 */
